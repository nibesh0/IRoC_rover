...
typedef struct
{
    std::string BlockingMode;

    int colorFormatNum;
    int colorWidthNum;
    int colorHeightNum;
    int colorActualFpsNum;

    int depthWidthNum;
    int depthHeightNum;
    int depthActualFpsNum;
    int depthDataTypeNum;
...
static void *property_func(void *arg);
...
static unsigned int gCameraVID = 0xffff;
...
ros::Publisher cloudpoint_img_pub;
sensor_msgs::PointCloud cloudpoint_image;

ros::Publisher camera_info_pub;
sensor_msgs::CameraInfo camera_info_data;

PointCloudInfo *pPointCloudInfo = NULL;
img_parameter imgParameter;

std::string filePath;

int main(int argc, char **argv)
{
    ros::init(argc, argv, "bimu_camera");
    ros::NodeHandle bimu_camera;

    bimu_camera.getParam("/bimu_rgbd_camera/BlockingMode", imgParameter.BlockingMode);
    
    bimu_camera.getParam("/bimu_rgbd_camera/colorFormatNum", imgParameter.colorFormatNum);
    bimu_camera.getParam("/bimu_rgbd_camera/colorWidthNum", imgParameter.colorWidthNum);
    bimu_camera.getParam("/bimu_rgbd_camera/colorHeightNum", imgParameter.colorHeightNum);
    bimu_camera.getParam("/bimu_rgbd_camera/colorActualFpsNum", imgParameter.colorActualFpsNum);

    bimu_camera.getParam("/bimu_rgbd_camera/depthWidthNum", imgParameter.depthWidthNum);
    bimu_camera.getParam("/bimu_rgbd_camera/depthHeightNum", imgParameter.depthHeightNum);
    bimu_camera.getParam("/bimu_rgbd_camera/depthActualFpsNum", imgParameter.depthActualFpsNum);
    bimu_camera.getParam("/bimu_rgbd_camera/depthDataTypeNum", imgParameter.depthDataTypeNum);
    bimu_camera.getParam("/bimu_rgbd_camera/buffSizeNum", imgParameter.buffSizeNum);

    bimu_camera.getParam("/bimu_rgbd_camera/colorImg", imgParameter.colorImg);
    bimu_camera.getParam("/bimu_rgbd_camera/depthImg", imgParameter.depthImg);
    bimu_camera.getParam("/bimu_rgbd_camera/pointCloudImg", imgParameter.pointCloudImg);
    bimu_camera.getParam("/bimu_rgbd_camera/camera_info", imgParameter.camera_info);

    bimu_camera.getParam("/bimu_rgbd_camera/AE", imgParameter.AE);

    bimu_camera.getParam("/bimu_rgbd_camera/irMaxValue", imgParameter.irMaxValue);
    bimu_camera.getParam("/bimu_rgbd_camera/currentIRValue", imgParameter.currentIRValue);

    bimu_camera.getParam("/bimu_rgbd_camera/frameSelection", imgParameter.frameSelection);

    bimu_camera.getParam("/bimu_rgbd_camera/devInfoPID", imgParameter.devInfoPID);
    bimu_camera.getParam("/bimu_rgbd_camera/devInfoVID", imgParameter.devInfoVID);
    bimu_camera.getParam("/bimu_rgbd_camera/devIndex", imgParameter.devIndex);

    bimu_camera.getParam("/bimu_rgbd_camera/file_path", imgParameter.file_path);

    bimu_camera.getParam("/bimu_rgbd_camera/pointCloudMaxFar", imgParameter.pointCloudMaxFar);

    filePath = imgParameter.file_path;

    int init_tmp = -5, devive_tmp = -5;
    init_tmp = init_device(true);
    if (init_tmp != 0)
        return -1;

    std::string image_raw = "/camera" + std::to_string(gsDevSelInfo.index) + "/infra/image_raw";
    std::string image_rect_raw = "/camera" + std::to_string(gsDevSelInfo.index) + "/depth/image_rect_raw";
    std::string point_cloud = "/camera" + std::to_string(gsDevSelInfo.index) + "/point_cloud";
    std::string camera_info = "/camera" + std::to_string(gsDevSelInfo.index) + "/camera_info";
...
static int init_device(bool is_select_dev)
{
    ...
    else
    {
        if (nDevCount > 1)
        {
            ...
            if(imgParameter.devIndex > nDevCount - 1)
            {
                printf("Device number out of range!!!\n");
                return -1;
            }
            else
                gsDevSelInfo.index = imgParameter.devIndex;
            printf("Selected deivce indxe is  = [%d]\n", gsDevSelInfo.index);
            gCameraPID = g_pDevInfo[gsDevSelInfo.index].wPID;
            gCameraVID = g_pDevInfo[gsDevSelInfo.index].wVID;
        }
        else
        {
            if(imgParameter.devIndex != 0)
            {
                printf("Device number out of range!!!\n");
                return -1;
            }
    ...
    if((gCameraPID != imgParameter.devInfoPID) || (gCameraVID != imgParameter.devInfoVID))
    {
        printf("PID or vid error!!!\n");
        return -1;
    }
...
static int open_device(void)
{
    ...
    else
    {
        ...
        // scanf("%s", input);
        input[0] = imgParameter.BlockingMode[0];
        ...
        gColorFormat = imgParameter.colorFormatNum;
        gColorWidth = imgParameter.colorWidthNum;
        gColorHeight = imgParameter.colorHeightNum;
        gActualFps = imgParameter.colorActualFpsNum;
        printf("F:%d    W:%d    H:%d    A:%d\n", imgParameter.colorFormatNum, imgParameter.colorWidthNum, 
                imgParameter.colorHeightNum, imgParameter.colorActualFpsNum);
        ...
        gDepthWidth = imgParameter.depthWidthNum;
        gDepthHeight = imgParameter.depthHeightNum;
        gActualFps = imgParameter.depthActualFpsNum;
...
static void *pfunc_thread_point_cloud(void *arg)
{
    ...
    while (1)
    {
        ...
        if ((ret == APC_OK) && (count%imgParameter.frameSelection == 0))
        {
            if(count > 2147483640)
                count = 0;
            ...
            // save_file(gColorImgBuf, gColorImgSize,gColorWidth,  gColorHeight, gColorFormat, true);//jpg
            // if(ret == APC_OK) {
            //     save_file(gColorRGBImgBuf, gColorWidth*gColorHeight*3, gColorWidth, gColorHeight, gColorFormat, false);
            // }
            if(imgParameter.colorImg == 1){
                rgb_image.header.stamp = ros::Time::now();
                rgb_image.height = gColorHeight;
                rgb_image.width = gColorWidth;
                rgb_image.encoding = "rgb8";
                rgb_image.is_bigendian = false;
                rgb_image.step = gColorWidth * 3;
                size_t rgb_size_pub = rgb_image.step * gColorHeight;
                rgb_image.data.resize(rgb_size_pub);

                memcpy((char *)(&rgb_image.data[0]), gColorRGBImgBuf /*gColorImgBuf*/, rgb_size_pub);
                rgb_img_pub.publish(rgb_image);
            }

            if(imgParameter.depthImg == 1){
                depth_image.header.stamp = ros::Time::now();
                depth_image.height = gDepthHeight;
                depth_image.width = gDepthWidth;
                depth_image.encoding = "mono16";
                depth_image.is_bigendian = false;
                depth_image.step = gDepthWidth * 2;
                size_t depth_size_pub = depth_image.step * gDepthHeight;
                depth_image.data.resize(depth_size_pub);

                memcpy((char *)(&depth_image.data[0]), gDepthImgBuf, depth_size_pub);
                depth_img_pub.publish(depth_image);
            }

            if(imgParameter.pointCloudImg == 1){
            ...
            if(imgParameter.camera_info == 1){
                camera_info_data.header.stamp = ros::Time::now();
                camera_info_data.height = gDepthHeight;
                camera_info_data.width = gDepthWidth;
  
                if(pPointCloudInfo == NULL){
                    pPointCloudInfo = new PointCloudInfo();
                    ret = getPointCloudInfo(EYSD, &gsDevSelInfo, pPointCloudInfo/*&pointCloudInfo*/, gDepthDataType, gDepthHeight);
                }
...
static int getPointCloudInfo(void *pHandleEYSD, DEVSELINFO *pDevSelInfo, PointCloudInfo *pointCloudInfo, int depthDataType, int depthHeight)
{
    ...
    if (ret == APC_OK)
    {
        ...
        camera_info_data.distortion_model = "plumb_bob";

        camera_info_data.D = {0, 0, 0, 0, 0};

        camera_info_data.K = {pointCloudInfo->focalLength, 0, pointCloudInfo->centerX,
                            0, pointCloudInfo->focalLength, pointCloudInfo->centerY,
                            0, 0, 1
        };

        camera_info_data.R = {1, 0, 0,
                            0, 1, 0,
                            0, 0, 1
        };

        camera_info_data.P = {pointCloudInfo->focalLength, 0, pointCloudInfo->centerX, 0,
                            0, pointCloudInfo->focalLength, pointCloudInfo->centerY, 0,
                            0, 0, 1, 0
                  ...
                  float fFar)
{
    ...
    if (ret == APC_OK)
    {
        ret = APC_GetPointCloud(pHandleEYSD, pDevSelInfo, ImgColor, CW, CH, ImgDepth, DW, DH,
                                pPointCloudInfo/*&pointCloudInfo*/, pPointCloudRGB, pPointCloudXYZ, fNear, fFar);
        if (ret == APC_OK)
        {
            ...
            cloudpoint_image.header.frame_id = "map";
...
static void setV4L2buffer()
{
    ...
    // scanf("%d", (int *)&value);
    value = imgParameter.buffSizeNum;
...
int getZDtable(DEVINFORMATION *pDevInfo, DEVSELINFO DevSelInfo, int depthHeight, int colorHeight)
{
    ...
    if (pDevInfo[DevSelInfo.index].nDevType == PUMA)
    { // 8052, 8053 is used to smae ZD table
    ...
    if (g_maxFar > imgParameter.pointCloudMaxFar)
        g_maxFar = imgParameter.pointCloudMaxFar;
...
static void setupDepth(void)
{
    ...
    gDepthDataType = imgParameter.depthDataTypeNum;
}
...
static int TransformDepthDataType(int *nDepthDataType, bool bRectifyData)
{
    ...
    case APC_DEPTH_DATA_8_BITS_RAW:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_8_BITS : APC_DEPTH_DATA_8_BITS_RAW;
        break;
    case APC_DEPTH_DATA_8_BITS_x80:
    case APC_DEPTH_DATA_8_BITS_x80_RAW:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_8_BITS_x80 : APC_DEPTH_DATA_8_BITS_x80_RAW;
        break;
    case APC_DEPTH_DATA_11_BITS:
    case APC_DEPTH_DATA_11_BITS_RAW:
    case APC_DEPTH_DATA_11_BITS_COMBINED_RECTIFY:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_11_BITS : APC_DEPTH_DATA_11_BITS_RAW;
        break;
    case APC_DEPTH_DATA_14_BITS:
    case APC_DEPTH_DATA_14_BITS_RAW:
    case APC_DEPTH_DATA_14_BITS_COMBINED_RECTIFY:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_14_BITS : APC_DEPTH_DATA_14_BITS_RAW;
        break;
    case APC_DEPTH_DATA_OFF_RAW:
    case APC_DEPTH_DATA_OFF_RECTIFY:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_OFF_RECTIFY : APC_DEPTH_DATA_OFF_RAW;
        break;
    default:
        *nDepthDataType = bRectifyData ? APC_DEPTH_DATA_DEFAULT : APC_DEPTH_DATA_OFF_RECTIFY;
        break;
    }
...
int setupIR(unsigned short IRvalue)
{
    ...
    if (m_nIRValue != 0)
    {
        ...
        ret = APC_SetIRMaxValue(EYSD, &gsDevSelInfo, imgParameter.irMaxValue);
        if (APC_OK != ret)
        {
            CT_DEBUG("set IR maxvlue fail.. (ret=%d)\n", ret);
        }
        CT_DEBUG("set IR Max success.. (ret=%d)\n", ret);

        ret = APC_SetCurrentIRValue(EYSD, &gsDevSelInfo, imgParameter.currentIRValue);
        if (APC_OK != ret)
        {
            CT_DEBUG("set IR value fail.. (ret=%d)\n", ret);
        }

#ifdef DEBUG_IR
    {
        unsigned short m_nIRMax, m_nIRMin,curIR;
        ...
        ret = APC_GetCurrentIRValue(EYSD, &gsDevSelInfo, &curIR);
        if (APC_OK != ret || m_nIRMin == 0xff)
        {
            CT_DEBUG("get IR Min value failed\n");
            return ret;
...
static void *property_func(void *arg)
{
    ...
    if (ret == APC_OK)
    {
        CT_DEBUG("AE[cur_val] = [0x%08x]\n", cur_val);
        // NOTE: The 0x01 means the 'EXPOSURE_MANUAL'
        if (cur_val != imgParameter.AE)
        {
            set_val = imgParameter.AE;
            ret = APC_SetCTPropVal(EYSD, &gsDevSelInfo, id, set_val);
            if (ret != APC_OK)
            {
                CT_DEBUG("Failed to call APC_SetCTPropVal() for (%d) !! (%d)\n", id, ret);
